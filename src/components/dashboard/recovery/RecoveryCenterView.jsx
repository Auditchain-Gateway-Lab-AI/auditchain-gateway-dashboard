  ...
  return (
    <section className="ac-recovery-center">
      <div className="ac-hero ac-recovery-center__hero">
        ...
        <div className="ac-hero__content">
          <div className="ac-hero__left">
            <span className="ac-page-kicker">Audit Logs / Recovery</span>
          ...
          <div className="ac-recovery-center__hero-action">
            <button type="button" className="ac-btn-ghost-action" onClick={() => navigate('/audit-logs')}><Icon name="history" size={15} /> Back to audit logs</button>
          </div>
        </div>
      </div>
      ...
      <div className="ac-recovery-stat-grid">
        ...
        <div className="ac-recovery-stat"><span className="ac-recovery-stat__icon ac-recovery-stat__icon--info"><Icon name="refresh" size={17} /></span><div><strong>{stats.recovering}</strong><span>Executing</span></div></div>
      ...
      <div className="ac-recovery-center__toolbar">
        <div className="ac-recovery-segmented" role="tablist" aria-label="Recovery data type">
          <button type="button" className={activeSection === 'incidents' ? 'is-active' : ''} onClick={() => setActiveSection('incidents')}>Incidents</button>
          <button type="button" className={activeSection === 'requests' ? 'is-active' : ''} onClick={() => setActiveSection('requests')}>Requests {stats.ready > 0 && <span>{stats.ready}</span>}</button>
        </div>
        <label className="ac-recovery-search"><Icon name="search" size={15} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search ID, resource, or reason" /></label>
        <button type="button" className="ac-btn-ghost-action" onClick={loadData} disabled={loading}><Icon name={loading ? 'spinner' : 'refresh'} size={14} /> Refresh</button>
      </div>

      {activeSection === 'incidents' ? (
        <div className="ac-recovery-center__panel">
          <div className="ac-recovery-center__panel-head">
            <div><h2>Tamper incidents</h2><p>Integrity incidents detected by the gateway scanner.</p></div>
            <select value={incidentFilter} onChange={event => setIncidentFilter(event.target.value)} aria-label="Filter incidents">
        ...
        <div className="ac-recovery-center__panel">
          <div className="ac-recovery-center__panel-head">
            <div><h2>Recovery requests</h2><p>Client-operated recovery queue for the active workspace.</p></div>
            <select value={requestFilter} onChange={event => setRequestFilter(event.target.value)} aria-label="Filter recovery requests">
